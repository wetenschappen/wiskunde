...
import { 
  PhX, PhArrowsOut, PhArrowRight, PhArrowLeft,
  PhCheckCircle
...
const baseShape = ref('triangle') // 'triangle', 'rectangle', 'square'
...
const showChallengeHint = ref(false)
...
// ==================== COMPUTED PROPERTIES ====================
// Display values that switch between analysis and challenge modes
const displayScaleFactor = computed(() => (currentMode.value === 'derivation' && challengeType.value === 'scale-calc') ? targetScale.value : scaleFactor.value);
const displayTriangleScale = computed(() => (currentMode.value === 'derivation' && challengeType.value === 'triangle-check') ? targetScale.value : triangleScale.value);

// Scale calculations
const scaledLength = computed(() => baseLength.value * displayScaleFactor.value)
const scaledWidth = computed(() => baseWidth.value * displayScaleFactor.value)
const areaScale = computed(() => displayScaleFactor.value ** 2)
const volumeScale = computed(() => displayScaleFactor.value ** 3)
...
const baseArea = computed(() => {
  ...
  if (baseShape.value === 'square') return baseLength.value ** 2
  return 0
})
...
const triangleB = computed(() => ({
  a: triangleA.value.a * displayTriangleScale.value,
  b: triangleA.value.b * displayTriangleScale.value,
  c: triangleA.value.c * displayTriangleScale.value
}))

const sideRatios = computed(() => {
  if (triangleA.value.a === 0 || triangleA.value.b === 0 || triangleA.value.c === 0) return [0, 0, 0];
  return [
...
function generateChallenge(type) {
  ...
  showChallengeHint.value = false
  
  if (type === 'scale-calc') {
    targetScale.value = Math.round((Math.random() * 2.5 + 0.5) * 10) / 10
    const baseL = Math.round(Math.random() * 4 + 2)
    targetLength.value = baseL
    targetArea.value = baseL * baseL
    userLengthAnswer.value = baseL // Pre-fill with base value
    userAreaAnswer.value = baseL * baseL // Pre-fill with base value
    ...
    targetScale.value = thalesRatio.value
...
function checkChallenge() {
  ...
  if (challengeType.value === 'scale-calc') {
    ...
    if (ratioCorrect) {
      ...
      challengeFeedback.value = 'Nog niet juist. De verhouding van de evenwijdige zijden is gelijk aan de schaalfactor.'
    }
  }
}
      ...
      <div class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0">
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center">
            <PhArrowsOut class="text-xl text-white" weight="bold" />
          </div>
          <div>
          ...
          <div class="flex bg-slate-100 rounded-lg p-1">
            <button 
              ...
              :class="['px-4 py-1.5 rounded-md text-sm font-medium transition-all', 
                currentMode === 'analysis' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700']"
            >
              Analyse
            </button>
            <button 
              ...
              :class="['px-4 py-1.5 rounded-md text-sm font-medium transition-all', 
                currentMode === 'derivation' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700']"
            >
              Oefeningen
            </button>
          </div>
          
          <button @click="emit('close')" aria-label="Sluiten" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors">
          ...
          <template v-if="currentMode === 'analysis'">
              ...
              <div class="flex gap-2">
                <button 
                  ...
                  :class="['flex-1 py-2 px-3 text-xs font-medium rounded-lg transition-all', 
                    analysisSubMode === 'scale' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200']"
                >
                  Schaal
                </button>
                <button 
                  ...
                  :class="['flex-1 py-2 px-3 text-xs font-medium rounded-lg transition-all', 
                    analysisSubMode === 'triangles' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200']"
                >
                  Driehoeken
                </button>
                <button 
                  ...
                  :class="['flex-1 py-2 px-3 text-xs font-medium rounded-lg transition-all', 
                    analysisSubMode === 'thales' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200']"
                >
                  Thales
                </button>
              </div>
            ...
            <div class="p-4 space-y-5">
              ...
              <template v-if="analysisSubMode === 'scale'">
                <div class="space-y-2">
                  <div class="flex justify-between items-center">
                    <label class="text-sm font-bold text-indigo-600">Schaalfactor k</label>
                  ...
                  <div class="flex gap-2">
                    <button 
                      ...
                      :class="['flex-1 py-2 px-3 text-sm rounded-lg border transition-all', 
                        baseShape === 'triangle' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-600 hover:border-slate-300']"
                    >
                      Driehoek
                    </button>
                    <button 
                      ...
                      :class="['flex-1 py-2 px-3 text-sm rounded-lg border transition-all', 
                        baseShape === 'rectangle' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-600 hover:border-slate-300']"
                    >
                      Rechthoek
                    </button>
                    <button 
                      @click="baseShape = 'square'"
                      :class="['flex-1 py-2 px-3 text-sm rounded-lg border transition-all', 
                        baseShape === 'square' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-200 text-slate-600 hover:border-slate-300']"
                    >
                      Vierkant
                    </button>
                  </div>
                ...
                <div v-if="currentSteps[currentStep].focus === 'area-effect' || currentSteps[currentStep].focus === 'comparison'" 
                  ...
                  <div class="flex justify-between text-sm">
                    ...
                    <span class="font-mono text-amber-600">{{ scaledLength.toFixed(2) }}</span>
                  </div>
                  <div class="border-t border-slate-200 pt-2 mt-2">
                    ...
                    <div class="flex justify-between text-sm">
                      ...
                      <span class="font-mono text-amber-600">{{ scaledArea.toFixed(2) }}</span>
                    </div>
              ...
              <template v-if="analysisSubMode === 'triangles'">
                ...
                <div class="flex items-center gap-4">
                  <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                    <input type="checkbox" v-model="showAngles" class="rounded text-amber-500 focus:ring-amber-500">
                    <span>Toon hoeken</span>
                  </label>
                  <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                    <input type="checkbox" v-model="showRatios" class="rounded text-amber-500 focus:ring-amber-500">
                ...
                <div v-if="currentSteps[currentStep].focus === 'ratios'" 
                  ...
                  <div class="space-y-1 text-sm font-mono">
                    <div class="flex justify-between">
                      ...
                      <span class="text-amber-600">{{ sideRatios[0].toFixed(2) }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-slate-600">b₂/b₁ =</span>
                      <span class="text-amber-600">{{ sideRatios[1].toFixed(2) }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-slate-600">c₂/c₁ =</span>
                      <span class="text-amber-600">{{ sideRatios[2].toFixed(2) }}</span>
                    </div>
                  </div>
                </div>
              </template>
              ...
              <template v-if="analysisSubMode === 'thales'">
                <div class="space-y-2">
                  <div class="flex justify-between items-center">
                    <label class="text-sm font-bold text-indigo-600">Verhouding AD/AB</label>
                ...
                <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                  <input type="checkbox" v-model="showParallel" class="rounded text-amber-500 focus:ring-amber-500">
                ...
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1 text-sm">
                  ...
                  <div class="flex justify-between border-t border-slate-200 pt-1 mt-1">
                    ...
                    <span class="font-mono text-amber-600">{{ thalesRatio.toFixed(2) }}</span>
                  </div>
                </div>
              </template>

              <!-- Educational details -->
              <div v-if="currentSteps[currentStep].details" class="p-3 bg-amber-50 rounded-lg border border-amber-200">
          ...
          <template v-if="currentMode === 'derivation'">
            ...
            <div class="p-4 space-y-4">
              ...
              <div v-if="!challengeActive" class="space-y-3">
                ...
                <button @click="generateChallenge('scale-calc')" class="w-full p-3 text-left border border-slate-200 rounded-lg hover:border-amber-500 hover:bg-amber-50 transition-all">
                ...
                <button @click="generateChallenge('triangle-check')" class="w-full p-3 text-left border border-slate-200 rounded-lg hover:border-amber-500 hover:bg-amber-50 transition-all">
                ...
                <button @click="generateChallenge('thales-calc')" class="w-full p-3 text-left border border-slate-200 rounded-lg hover:border-amber-500 hover:bg-amber-50 transition-all">
              ...
              <div v-else class="space-y-4">
                <div class="p-3 bg-indigo-700 text-white rounded-lg">
                  <p class="text-sm font-medium mb-1">Bepaal de juiste waarden:</p>
                  
                  <div v-if="challengeType === 'scale-calc'" class="text-xs text-indigo-200 space-y-1">
                    <p>Een figuur wordt vergroot met schaalfactor k = {{ targetScale }}</p>
                    <p>Basis lengte: {{ targetLength }}, basis oppervlakte: {{ targetArea.toFixed(2) }}</p>
                    <div class="mt-2">
                      <button v-if="!showChallengeHint" @click="showChallengeHint = true" class="text-xs text-indigo-300 underline hover:text-white transition-colors">Toon tip</button>
                      <p v-else class="text-indigo-300">Tip: lengte' = lengte × k, opp.' = opp. × k²</p>
                    </div>
                  </div>
                  
                  <div v-if="challengeType === 'triangle-check'" class="text-xs text-indigo-200 space-y-1">
                    <p>Twee driehoeken zijn gelijkvormig.</p>
                    <p>Bepaal de schaalfactor k.</p>
                    <div class="mt-2">
                      <button v-if="!showChallengeHint" @click="showChallengeHint = true" class="text-xs text-indigo-300 underline hover:text-white transition-colors">Toon tip</button>
                      <p v-else class="text-indigo-300">Tip: vergelijk de lengtes van overeenkomstige zijden.</p>
                    </div>
                  </div>
                  
                  <div v-if="challengeType === 'thales-calc'" class="text-xs text-indigo-200 space-y-1">
                    <p>In de driehoek is DE evenwijdig aan BC.</p>
                    <p>De schaalfactor k is {{ targetScale.toFixed(2) }}. Wat is de verhouding DE/BC?</p>
                    <div class="mt-2">
                      <button v-if="!showChallengeHint" @click="showChallengeHint = true" class="text-xs text-indigo-300 underline hover:text-white transition-colors">Toon tip</button>
                      <p v-else class="text-indigo-300">Tip: DE/BC = AD/AB = AE/AC = k</p>
                    </div>
                  </div>
                </div>
                ...
                <!-- Actions -->
                <div class="space-y-2">
                  <button @click="checkChallenge" class="w-full py-2 bg-indigo-600 text-white rounded-lg font-medium text-sm hover:bg-indigo-700 transition-colors">
                    Controleer
                  </button>
          ...
          <div class="mt-auto p-4 bg-slate-50 border-t border-slate-200">
            ...
            <template v-if="analysisSubMode === 'scale' || (currentMode === 'derivation' && challengeType === 'scale-calc')">
              <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                  ...
                  <span class="font-mono text-amber-600">k = {{ displayScaleFactor.toFixed(2) }}</span>
                </div>
                <div class="flex justify-between">
            ...
            <template v-if="analysisSubMode === 'triangles' || (currentMode === 'derivation' && challengeType === 'triangle-check')">
              <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                  <span class="text-slate-600">Schaalfactor:</span>
                  <span class="font-mono text-amber-600">k = {{ displayTriangleScale.toFixed(2) }}</span>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                  ...
                  <span class="font-mono">{{ (displayTriangleScale * (triangleA.a + triangleA.b + triangleA.c)).toFixed(2) }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-slate-600">Oppervlakte Δ₂:</span>
                  <span class="font-mono">{{ (displayTriangleScale ** 2 * (triangleA.a * triangleA.b / 2)).toFixed(2) }}</span>
                </div>
              </div>
            </template>
            ...
            <template v-if="analysisSubMode === 'thales' || (currentMode === 'derivation' && challengeType === 'thales-calc')">
              <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                  <span class="text-slate-600">Schaal kleine Δ:</span>
                  <span class="font-mono">{{ thalesRatio.toFixed(2) }}</span>
                </div>
              </div>
            </template>
            ...
            <div class="bg-white rounded-xl p-4 mb-4 shadow-sm border border-slate-200">
              <div class="flex items-center justify-center gap-6">
                <div class="text-center">
                  ...
                  <span class="text-2xl font-mono font-bold text-amber-500">k = {{ displayScaleFactor.toFixed(2) }}</span>
                </div>
            ...
            <div class="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 relative overflow-hidden">
              ...
              >
                ...
                <g v-if="baseShape === 'triangle'">
                  <path 
                    :d="generateTrianglePath({a: baseLength, b: baseWidth}, -6, -3)"
                    fill="none"
                    stroke="#3B82F6"
                    stroke-width="3"
                  />
                ...
                <g v-if="baseShape === 'triangle'">
                  <path 
                    :d="generateTrianglePath({a: scaledLength, b: scaledWidth}, 1, -3)"
              ...
              <div class="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm p-3 rounded-lg border border-slate-200 shadow-sm">
                <div class="space-y-1.5 text-xs">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-slate-600">Geschaald (k = {{ displayScaleFactor.toFixed(2) }})</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <template v-if="analysisSubMode === 'triangles' || (currentMode === 'derivation' && challengeType === 'triangle-check')">
            <div class="bg-white rounded-xl p-4 mb-4 shadow-sm border border-slate-200">
              <div class="flex items-center justify-center gap-6">
                ...
                <span class="text-xl font-mono font-bold text-amber-500">ΔABC ~ ΔA'B'C'</span>
              </div>
            </div>
            ...
            <div class="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 relative overflow-hidden">
              ...
              >
                ...
                <!-- Side labels -->
                <text :x="toSvgX(-6 + triangleA.b/2)" :y="toSvgY(-4) + 15" text-anchor="middle" class="fill-blue-500 text-xs">{{ triangleA.b }}</text>
                <text :x="toSvgX(-6) - 20" :y="toSvgY(-4 + triangleA.a/2)" text-anchor="middle" class="fill-blue-500 text-xs">{{ triangleA.a }}</text>
                
                <text :x="toSvgX(2 + triangleB.b/2)" :y="toSvgY(-4) + 15" text-anchor="middle" class="fill-orange-500 text-xs">{{ triangleB.b.toFixed(1) }}</text>
                <text :x="toSvgX(2) - 25" :y="toSvgY(-4 + triangleB.a/2)" text-anchor="middle" class="fill-orange-500 text-xs">{{ triangleB.a.toFixed(1) }}</text>

                <!-- Ratio annotations -->
                <g v-if="showRatios">
                  <text :x="toSvgX(-2)" :y="toSvgY(-5)" text-anchor="middle" class="fill-slate-500 text-xs">k = {{ displayTriangleScale.toFixed(2) }}</text>
                ...
                <g v-if="showAngles">
                  <path :d="`M ${toSvgX(-6 + 0.5)} ${toSvgY(-4)} A 20 20 0 0 1 ${toSvgX(-6)} ${toSvgY(-4 + 0.5)}`" fill="none" stroke="#3B82F6" stroke-width="1.5"/>
                  <path :d="`M ${toSvgX(2 + 0.5 * displayTriangleScale)} ${toSvgY(-4)} A ${20 * displayTriangleScale} ${20 * displayTriangleScale} 0 0 1 ${toSvgX(2)} ${toSvgY(-4 + 0.5 * displayTriangleScale)}`" fill="none" stroke="#F97316" stroke-width="1.5"/>
                </g>
              ...
              <div class="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm p-3 rounded-lg border border-slate-200 shadow-sm">
                <div class="space-y-1.5 text-xs">
                  ...
                  <div class="pt-1 border-t border-slate-200 mt-1">
                    <span class="text-slate-500">Schaalfactor: k = {{ displayTriangleScale.toFixed(2) }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <template v-if="analysisSubMode === 'thales' || (currentMode === 'derivation' && challengeType === 'thales-calc')">
            <div class="bg-white rounded-xl p-4 mb-4 shadow-sm border border-slate-200">
              <div class="flex items-center justify-center gap-6">
                ...
                <span class="text-lg font-mono text-amber-500">DE ∥ BC ⇒ ΔADE ~ ΔABC</span>
              </div>
            </div>
...
input[type="range"]::-webkit-slider-thumb {
  ...
  background: #4f46e5; /* Default color, can be overridden by accent-xxx */
}
...
/* Smoothly animate geometry when sliders are modified */
svg path, svg line {

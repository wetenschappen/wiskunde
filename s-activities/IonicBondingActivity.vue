...
const layout = computed(() => {
  ...
  if (moleculeKey === 'NaCl') {
    ...
    const targetY = cl.y + 10 // Arrive slightly below center
    const arc = -30
...
const electronTrails = computed(() => {
  ...
  layout.value.movingElectrons.forEach((electron) => {
    ...
    for (let i = 1; i <= trailLength; i++) {
      ...
      if (trailProgress > 0) {
        // Must use the same easing function as the main animation
        const eased = trailProgress < 0.5 
          ? 2 * trailProgress * trailProgress 
          : 1 - Math.pow(-2 * trailProgress + 2, 2) / 2
        
        // Use the same arc calculation as the main electron
  ...
  <Transition name="fade">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex flex-col bg-slate-950 text-slate-50 font-sans">
      ...
      <div class="h-16 bg-slate-900 border-b border-white/10 flex items-center justify-between px-6 shrink-0">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-sm text-slate-400">Elektronen-overdracht van metaal naar niet-metaal</p>
          </div>
        </div>
      ...
      <div class="flex-1 flex overflow-hidden">
          ...
          <div class="p-6 flex-1 flex flex-col gap-6">
            ...
            <div class="grid grid-cols-3 gap-2">
              <button v-for="(_, key) in moleculeData" :key="key" @click="selectMolecule(key)"
                class="px-4 py-2 text-sm font-bold rounded-lg transition-all duration-300"
                :class="selectedMolecule === key ? 'bg-indigo-600 text-white shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-slate-700/50 hover:bg-slate-700 text-slate-300'">
            ...
            <div class="flex flex-col gap-4 flex-1">
              ...
              <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
                <h3 class="text-xl font-bold text-white mb-2">{{ currentMoleculeInfo.name }} ({{ currentMoleculeInfo.formula }})</h3>
                <p class="text-slate-400 text-sm leading-relaxed">{{ currentMoleculeInfo.description }}</p>
              </div>

              <!-- STEP-BY-STEP PANELS -->
              <div class="space-y-4 overflow-y-auto pr-2 -mr-2">
                <div class="bg-slate-800/80 border border-slate-700 rounded-xl p-5 relative overflow-hidden transition-all duration-500"
                     :class="animationPhase === 0 ? 'ring-2 ring-amber-500/50' : ''">
                  <div class="absolute left-0 top-0 bottom-0 w-1" :class="animationPhase === 0 ? 'bg-amber-500' : 'bg-slate-700'"></div>
                  <h4 class="font-bold text-white flex items-center gap-2 mb-2"><PhInfo class="text-amber-400"/> Stap 1: Het Begin</h4>
                  <p class="text-sm text-slate-300">De atomen zijn in hun neutrale grondtoestand, met hun valentie-elektronen.</p>
                </div>
                
                <div class="bg-slate-800/80 border border-slate-700 rounded-xl p-5 relative overflow-hidden transition-all duration-500"
                     :class="animationPhase === 1 ? 'ring-2 ring-rose-500/50' : ''">
                  <div class="absolute left-0 top-0 bottom-0 w-1" :class="animationPhase === 1 ? 'bg-rose-500' : 'bg-slate-700'"></div>
                  <h4 class="font-bold text-rose-400 flex items-center gap-2 mb-2"><PhArrowsDownUp /> Stap 2: Transfer</h4>
                  <p class="text-sm text-slate-300">Het metaal geeft zijn valentie-elektron(en) af aan het niet-metaal. Kijk hoe de elektronen bewegen!</p>
                </div>

                <div class="bg-slate-800/80 border border-slate-700 rounded-xl p-5 relative overflow-hidden transition-all duration-500"
                     :class="animationPhase === 2 ? 'ring-2 ring-emerald-500/50' : ''">
                  <div class="absolute left-0 top-0 bottom-0 w-1" :class="animationPhase === 2 ? 'bg-emerald-500' : 'bg-slate-700'"></div>
                  <h4 class="font-bold text-emerald-400 flex items-center gap-2 mb-2"><PhCheckCircle /> Stap 3: Ionen gevormd</h4>
                  <p class="text-sm text-slate-300">Er zijn nu ionen gevormd. De positieve en negatieve ladingen trekken elkaar sterk aan.</p>
                </div>
              </div>
            </div>
            ...
            <button 
              @click="playAnimation" 
              class="w-full mt-auto py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] transition-all flex items-center justify-center gap-2 active:scale-95"
        ...
        <!-- SVG CANVAS -->
        <div class="flex-1 relative bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-slate-950 flex items-center justify-center">
            ...
            <svg viewBox="0 0 400 400" class="w-full h-full drop-shadow-2xl overflow-visible">
              ...
              <g v-for="atom in layout.atoms" :key="atom.id">
                ...
                <g v-if="animationPhase >= 2" class="animate-fade-in">
                  ...
                  <path :d="`M ${atom.x - 20} ${atom.y - 30} H ${atom.x - 28} V ${atom.y + 30} H ${atom.x - 20}`" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="2" v-if="atom.symbol !== 'Cl'"/>
                  <path :d="`M ${atom.x + 20} ${atom.y - 30} H ${atom.x + 28} V ${atom.y + 30} H ${atom.x + 20}`" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="2" v-if="atom.symbol !== 'Cl'"/>
                  <!-- Brackets for Cl ions -->
                  <path :d="`M ${atom.x - 40} ${atom.y - 45} H ${atom.x - 50} V ${atom.y + 45} H ${atom.x - 40}`" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="2" v-if="atom.symbol === 'Cl'"/>
                  <path :d="`M ${atom.x + 40} ${atom.y - 45} H ${atom.x + 50} V ${atom.y + 45} H ${atom.x + 40}`" fill="none" stroke="rgba(255,255,255,0.4)" stroke-width="2" v-if="atom.symbol === 'Cl'"/>

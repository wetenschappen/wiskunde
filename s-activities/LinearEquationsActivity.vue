...
const formulaType = ref('simple') // 'simple', 'physics', 'geometry'
...
const formulaChallenges = [
  ...
  },
  {
    ...
    solveFor: 't',
    steps: [
      { instruction: 'Trek v₀ af van beide leden', expression: 'v - v₀ = at' },
      { instruction: 'Deel beide leden door a', expression: 't = (v - v₀) / a' }
    ],
    answer: '(v - v₀) / a'
  },
  {
...
function generateFormulaChallenge() {
  ...
  const types = ['simple', 'physics', 'geometry']
            ...
            <div class="p-4 space-y-5">
              ...
              <template v-if="equationMode === 'formula'">
                <div class="space-y-4">
                  <div class="flex gap-2">
                    <button 
                      v-for="type in ['simple', 'physics', 'geometry']" 
                      :key="type"
                    ...
                    >
                      {{ type === 'simple' ? 'Eenvoudig' : type === 'physics' ? 'Natuurkunde' : 'Meetkunde' }}
                    </button>
                  </div>

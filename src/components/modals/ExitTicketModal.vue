...
const actualQuestionNumber = computed(() => {
  // Tel hoeveel MC, OPEN of GRAPH vragen we al gepasseerd zijn
  let count = 0
  for (let i = 0; i <= currentStepIndex.value; i++) {
    if (['mc', 'open', 'graph-point'].includes(parts.value[i].type)) {
      count++
    }
  }
  return count
        ...
        <div class="p-8 flex-1 flex flex-col overflow-y-auto">
            ...
            <Transition name="fade-slide" mode="out-in">
                ...
                <div v-if="currentPart.type === 'mc'" :key="'mc-'+currentPart.id" class="flex-1 flex flex-col">
                    <h4 class="text-xl font-bold text-slate-900 mb-6">{{ currentPart.question }}</h4>
                ...
                <div v-else-if="currentPart.type === 'open'" :key="'open-'+currentPart.id" class="flex-1 flex flex-col">
                    <h4 class="text-xl font-bold text-slate-900 mb-2">{{ currentPart.question }}</h4>
                    <p class="text-slate-500 text-sm mb-6">{{ currentPart.description }}</p>
                ...
                <!-- GRAPH POINT -->
                <div v-else-if="currentPart.type === 'graph-point'" :key="'graph-'+currentPart.id" class="flex-1 flex flex-col">
                    <h4 class="text-xl font-bold text-slate-900 mb-2">{{ currentPart.question }}</h4>
                    <p class="text-slate-500 text-sm mb-4">{{ currentPart.description }}</p>
                    
                    <div class="flex-1 border-2 border-slate-200 rounded-xl overflow-hidden bg-white relative cursor-crosshair min-h-[300px]">
                        <MathGraphSvg 
                            :config="getGraphConfig(currentPart)" 
                            @graph-click="(pt) => selectGraphPoint(currentPart.id, pt)" 
                        />
                        <div v-if="answers[currentPart.id]" class="absolute bottom-4 left-4 bg-white/90 backdrop-blur px-3 py-2 rounded-lg shadow-lg border border-slate-100 text-xs font-mono font-bold text-slate-700 flex items-center gap-2">

...
// Level Logic — now dynamically generated
const currentInternalLevel = ref(0)
const totalInternalLevels = 3
const levels = ref([])

function generateLevel() {
  const l1Pool = [4.1, 4.2, 4.3, 4.4]
  const l2Pool = [4.6, 4.7, 4.8, 4.9]
  const l1Pick = l1Pool[Math.floor(Math.random() * l1Pool.length)]
  const l2Pick = l2Pool[Math.floor(Math.random() * l2Pool.length)]
  return [
    {
      goalText: 'Opdracht 1: Afronden naar beneden',
      targetDecimal: l1Pick,
      exactAns: 4,
      hintPos: 'Je bal staat niet op ' + l1Pick + '. Zet hem eerst goed.',
      hintAns: 'Kijk naar de bal op ' + l1Pick + '. Welk dal ligt het dichtstbij? Naar welk cijfer rolt hij terug?'
    },
    {
      goalText: 'Opdracht 2: Afronden naar boven',
      targetDecimal: l2Pick,
      exactAns: 5,
      hintPos: 'Zet de bal eerst op ' + l2Pick + ' zoals gevraagd in de opdracht.',
      hintAns: 'De bal op ' + l2Pick + ' is al lang over de top. Welk dal is nu het dichtstbij?'
    },
    {
  ...
  if (userAns.value === data.exactAns) {
    if (isDropped.value && decimalValue.value === data.targetDecimal) {
        ...
        if (data.targetDecimal === 4.5) {
            feedback.value = { type: 'success', text: 'Prima!! Omdat 4.5 perfect in het midden ligt, hebben wiskundigen afgesproken om het altijd naar boven (5) af te ronden.'}
        } else {
        ...
        if (attemptCount.value <= 1) {
            ...
            feedback.value = { type: 'error', text: 'Niet helemaal... Sleep de slider naar ' + data.targetDecimal + ' en klik op "Laat Los" om te zien hoe de bal rolt.' }
        } else {
            feedback.value = { type: 'error', text: 'Niet helemaal... Zet de slider op ' + data.targetDecimal + ' en laat de bal los. Het juiste antwoord is al ' + data.exactAns + '!' }
        }
    }
  } else {
    isCorrect.value = false
    ...
    if (decimalValue.value !== data.targetDecimal) {
        if (attemptCount.value <= 1) {
            ...
            feedback.value = { type: 'error', text: 'Niet helemaal... Sleep de slider naar ' + data.targetDecimal + '. Je ziet de rode waarde boven de slider veranderen.' }
        } else {
            feedback.value = { type: 'error', text: 'Niet helemaal... Zet de slider precies op ' + data.targetDecimal + ', laat de bal los, en typ het getal waar de bal naartoe rolt.' }
        }
    } else {
        if (attemptCount.value <= 1) {
            feedback.value = { type: 'error', text: data.hintAns }
        } else if (attemptCount.value === 2) {
            feedback.value = { type: 'error', text: 'Niet helemaal... Kijk naar de heuvel. Aan welke kant ligt ' + data.targetDecimal + ' het dichtst bij een dal (4 of 5)?' }
        } else {
            feedback.value = { type: 'error', text: data.targetDecimal + ' ligt het dichtst bij ' + data.exactAns + '. Typ ' + data.exactAns + ' in als antwoord.' }
        }
    }
  }
}
              ...
              <div class="w-full max-w-3xl flex flex-col items-center">
                  ...
                  <div class="mb-12 w-full max-w-md bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col items-center z-20">
                      ...
                      <button @click="dropBall" :disabled="isDropped || decimalValue !== currentLevelData.targetDecimal"
                              class="mt-6 px-6 py-2 bg-slate-800 text-white font-bold rounded-lg shadow-sm flex items-center gap-2 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95 focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">

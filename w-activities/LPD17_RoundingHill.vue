  ...
  if (userAns.value === data.exactAns) {
    if (isDropped.value && decimalValue.value === data.targetDecimal) {
        ...
        if (attemptCount.value <= 1) {
            ...
            feedback.value = { type: 'error', text: 'Sleep de slider naar ' + data.targetDecimal + ' en klik op "Laat Los" om te zien hoe de bal rolt.' }
        } else {
            feedback.value = { type: 'error', text: 'Zet de slider op ' + data.targetDecimal + ' en laat de bal los. Het juiste antwoord is al ' + data.exactAns + '!' }
        }
    }
  } else {
    isCorrect.value = false
    ...
    if (decimalValue.value !== data.targetDecimal) {
        if (attemptCount.value <= 1) {
            ...
            feedback.value = { type: 'error', text: 'Sleep de slider naar ' + data.targetDecimal + '. Je ziet de rode waarde boven de slider veranderen.' }
        } else {
            feedback.value = { type: 'error', text: 'Zet de slider precies op ' + data.targetDecimal + ', laat de bal los, en typ het getal waar de bal naartoe rolt.' }
        }
    } else {
        if (attemptCount.value <= 1) {
            feedback.value = { type: 'error', text: data.hintAns }
        } else if (attemptCount.value === 2) {
            feedback.value = { type: 'error', text: 'Kijk naar de heuvel. Aan welke kant ligt ' + data.targetDecimal + ' het dichtst bij een dal (4 of 5)?' }
        } else {

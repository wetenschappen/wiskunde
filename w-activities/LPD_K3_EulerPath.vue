...
// A(top): degree 2
// B(mid-left): degree 3
// C(mid-right): degree 3
// D(bot-left): degree 2
// E(bot-right): degree 2
// Wait, classic house:
// Bot-left to Bot-right, Bot-right to Mid-right, Mid-right to Mid-left, Mid-left to Bot-left
// Crosses: Bot-left to Mid-right, Bot-right to Mid-left.
// Roof: Mid-left to Top, Mid-right to Top.
// Degrees: Top=2. Bot-left=3, Bot-right=3. Mid-left=4, Mid-right=4.
// Odd nodes: 2 (Bot-left, Bot-right).
// => Euler path exists!

const correctOddNodes = 2
const correctHasPath = 'Ja'
...
function checkAnswer() {
  ...
  if (parseInt(userOdd.value) === correctOddNodes && userHasPath.value === correctHasPath) {
    isCorrect.value = true
    feedback.value = { 
      type: 'success', 
      text: 'Perfect! Omdat er exact 2 knopen met een oneven graad zijn, is er een Eulerpad mogelijk.' 
    }
  } else if (parseInt(userOdd.value) !== correctOddNodes) {
    isCorrect.value = false
    feedback.value = { 
      type: 'error', 
      text: 'Tel de graden opnieuw. Hoeveel knopen hebben 1, 3, of 5 verbindingen?'
    }
  } else {
    isCorrect.value = false
    feedback.value = { 
      type: 'error', 
      text: 'Het aantal oneven knopen klopt, maar je conclusie is fout. De stelling van Euler zegt dat er een pad is bij 0 of 2 oneven knopen.'
    }
  }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-3">
              ...
              <button v-if="!isCorrect" @click="checkAnswer" :disabled="isChecked && !isCorrect || !userOdd || !userHasPath" class="flex-1 py-3 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 disabled:opacity-50 active:scale-[0.98]">
                Controleer
              </button>

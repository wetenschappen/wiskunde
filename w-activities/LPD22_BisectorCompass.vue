...
const levels = [
  {
    ...
    pB: { x: 450, y: 250 },
    radius: 200,
    intersectTop: { x: 300, y: 117.7 },
    intersectBot: { x: 300, y: 382.3 },
    mid: { x: 300, y: 250 },
    lineRotation: 0
  },
  {
    goalText: 'Opdracht 2: Verticaal Lijnstuk',
    pA: { x: 300, y: 100 },
    pB: { x: 300, y: 400 },
    radius: 200,
    intersectTop: { x: 167.7, y: 250 },
    intersectBot: { x: 432.3, y: 250 },
    mid: { x: 300, y: 250 },
    lineRotation: 90
  },
  {
    goalText: 'Opdracht 3: Schuin Lijnstuk',
    pA: { x: 150, y: 100 },
    pB: { x: 450, y: 400 }, // dx=300, dy=300, length=424.26, half=212.13
    radius: 250,
    // Midpoint is (300, 250)
    // Distance from mid to intersection h = sqrt(radius^2 - half^2) = sqrt(62500 - 45000) = sqrt(17500) = 132.28
    // Angle of AB is 45 deg (PI/4). Perpendicular angle is 135 deg (3*PI/4).
    // dx_int = h * cos(135) = 132.28 * (-0.707) = -93.5
    // dy_int = h * sin(135) = 132.28 * 0.707 = 93.5
    intersectTop: { x: 300 - 93.5, y: 250 + 93.5 }, // (206.5, 343.5)
    intersectBot: { x: 300 + 93.5, y: 250 - 93.5 }, // (393.5, 156.5)
    mid: { x: 300, y: 250 },
    lineRotation: 45
  }
]
...
function drawCircles() {
    ...
    if (!compassRadiusSet.value) {
        feedback.value = { type: 'error', text: 'Stel eerst de passeropening (straal) in!' }
        return
    }
...
function drawLine() {
    ...
    if (!circlesDrawn.value) {
        feedback.value = { type: 'error', text: 'Je moet eerst de snijpunten zoeken met de passer!' }
        return
    }
              ...
              <div class="relative bg-white shadow-xl rounded-2xl overflow-hidden border-2 border-slate-200 p-8" :key="'lvl' + currentInternalLevel">
                  <svg width="600" height="500" viewBox="0 0 600 500">
                      ...
                      <g v-if="lineDrawn" class="animate-fadeIn">
                          <!-- Extend line far beyond intersections -->
                          ...
                          <!-- Right angle symbol via rotation from midpoint -->
                          <!-- Needs rotation around midpoint depending on line rotation -->
                          <!-- A bit tricky in raw SVG without proper transforms, let's use a group with transform -->
                          <g :transform="`translate(${currentLevelData.mid.x}, ${currentLevelData.mid.y}) rotate(${currentLevelData.lineRotation})`">
                              <!-- Assuming the original line was horizontal, the bisector is vertical. So angle indicator at 90 deg -->
                              <!-- Original horizontal line means lineRotation = 0 -->
                              <!-- Base line is along X axis. Bisector is along Y axis. -->
                              <!-- Draw right angle in the top-right quadrant (x>0, y<0) -->
                              <polyline points="0,-15 15,-15 15,0" fill="none" stroke="#10b981" stroke-width="3" />
                              
                              <!-- Equality ticks on base line (X axis) -->

...
const levels = [
  ...
  {
    goalText: 'Opdracht 3: Teken de SCALAIRE vermenigvuldiging (v = -2a).',
    vecA: { x: -2, y: 1 },
    vecB: null, // not used
    op: '-2a',
    target: { x: 4, y: -2 }
...
function checkAnswer() {
  ...
  if (!userPoint.value) {
    isCorrect.value = false
    feedback.value = { type: 'error', text: 'Klik eerst op het rooster om de vector te tekenen.' }
    return
  }

  const target = currentLevelData.value.target;

  if (userPoint.value.x === target.x && userPoint.value.y === target.y) {
    ...
    // Scaffolding hints
    if (userPoint.value.x !== target.x && userPoint.value.y === target.y) {
      feedback.value = { type: 'error', text: `Je hoogte (y) klopt perfect, maar je horizontale positie is verkeerd. Je koos x=${userPoint.value.x}. Herbereken de x-waarden.` }
    } else if (userPoint.value.x === target.x && userPoint.value.y !== target.y) {
      feedback.value = { type: 'error', text: `Je breedte (x) klopt perfect, maar je verticale positie is verkeerd. Je koos y=${userPoint.value.y}. Herbereken de y-waarden.` }
    } else {
      feedback.value = { type: 'error', text: `Zowel je x- als y-coördinaat kloppen niet. Je tekende (${userPoint.value.x}, ${userPoint.value.y}). Pas de wiskundige bewerking (${currentLevelData.value.op}) toe op de componenten.` }
    }
  }
}

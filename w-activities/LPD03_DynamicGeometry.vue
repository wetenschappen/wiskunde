...
const levels = [
  {
    ...
    targetRule: (angles) => Math.max(...angles) > 90.5,
    hint: 'Eén van de hoeken moet groter dan 90° zijn.'
  },
  {
    goalText: 'Opdracht 2: Maak een rechthoekige driehoek.',
    targetRule: (angles) => angles.some(a => Math.abs(a - 90) < 1.5),
    hint: 'Eén van de hoeken moet exact (of heel dichtbij) 90° zijn.'
  },
  {
    goalText: 'Opdracht 3: Maak een scherphoekige driehoek.',
    targetRule: (angles) => Math.max(...angles) < 89.5,
    hint: 'ALLE drie de hoeken moeten kleiner dan 90° zijn.'
  }
]
...
function endDrag() { draggingPoint.value = null }
function onDrag(e) {
...
function checkAnswer() {
  ...
  if (passed) {
    ...
    isCorrect.value = false
    feedback.value = { type: 'error', text: `Dat is niet de gevraagde driehoek. ${currentLevelData.value.hint}`}
  }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-4 mt-6 border border-slate-200 bg-slate-50 rounded-xl shadow-inner">
               ...
               <ul class="space-y-2 text-sm font-mono">
                   <!-- Highlight rule based on level: e.g. level 0 highlights > 90, level 1 highlights ~90 -->
                   <li class="flex justify-between" :class="currentInternalLevel === 0 && angleA > 90 ? 'text-indigo-600 font-bold' : (currentInternalLevel === 1 && Math.abs(angleA-90) < 1.5 ? 'text-emerald-600 font-bold' : 'text-slate-600')"><span>∠A:</span> <span>{{ angleA.toFixed(1) }}°</span></li>
                   <li class="flex justify-between" :class="currentInternalLevel === 0 && angleB > 90 ? 'text-indigo-600 font-bold' : (currentInternalLevel === 1 && Math.abs(angleB-90) < 1.5 ? 'text-emerald-600 font-bold' : 'text-slate-600')"><span>∠B:</span> <span>{{ angleB.toFixed(1) }}°</span></li>
                   <li class="flex justify-between" :class="currentInternalLevel === 0 && angleC > 90 ? 'text-indigo-600 font-bold' : (currentInternalLevel === 1 && Math.abs(angleC-90) < 1.5 ? 'text-emerald-600 font-bold' : 'text-slate-600')"><span>∠C:</span> <span>{{ angleC.toFixed(1) }}°</span></li>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-3">
              ...
              <button v-if="!isCorrect" @click="checkAnswer" class="flex-1 py-3 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 active:scale-[0.98]">Controleer Hoeken</button>

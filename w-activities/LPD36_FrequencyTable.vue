        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-4 mt-6 border border-surface-200 bg-math-blue-bg rounded-xl shadow-inner">
               <h4 class="font-bold text-math-blue mb-4">Tabel (Absolute Freq.):</h4>

               <div class="flex flex-col gap-4">
                   <div class="flex items-center justify-between">
                       <span class="font-bold text-slate-700 w-20">Hond</span>
                       <input type="number" v-model.number="freqDog" :disabled="isCorrect" class="w-20 p-2 border-2 rounded-lg focus:border-math-blue outline-none text-center font-bold" :class="freqDog === currentLevel.dogCount && isChecked ? 'border-emerald-500 text-emerald-600' : 'border-surface-200'" />
                   </div>
                   <div class="flex items-center justify-between">
                       <span class="font-bold text-slate-700 w-20">Kat</span>
                       <input type="number" v-model.number="freqCat" :disabled="isCorrect" class="w-20 p-2 border-2 rounded-lg focus:border-math-blue outline-none text-center font-bold" :class="freqCat === currentLevel.catCount && isChecked ? 'border-emerald-500 text-emerald-600' : 'border-surface-200'" />
                   </div>
                   <div class="flex items-center justify-between">
                       <span class="font-bold text-slate-700 w-20">Vis</span>
                       <input type="number" v-model.number="freqFish" :disabled="isCorrect" class="w-20 p-2 border-2 rounded-lg focus:border-math-blue outline-none text-center font-bold" :class="freqFish === currentLevel.fishCount && isChecked ? 'border-emerald-500 text-emerald-600' : 'border-surface-200'" />
                   </div>
                   <div class="flex items-center justify-between">
                       <span class="font-bold text-slate-700 w-20">Vogel</span>
                       <input type="number" v-model.number="freqBird" :disabled="isCorrect" class="w-20 p-2 border-2 rounded-lg focus:border-math-blue outline-none text-center font-bold" :class="freqBird === currentLevel.birdCount && isChecked ? 'border-emerald-500 text-emerald-600' : 'border-surface-200'" />
                   </div>
               </div>
            </div>
          </div>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-if="!isCorrect" @click="drawGraph" class="flex-1 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 active:scale-[0.98] flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">
        ...
        <div class="flex flex-col flex-1 overflow-hidden bg-slate-50">
          <div class="flex flex-col flex-1 p-6 overflow-y-auto items-center justify-center relative pattern-grid">
              ...
              <div class="w-full max-w-4xl flex items-start justify-center gap-12">
                  ...
                  <div class="w-64 bg-white p-6 rounded-xl shadow-md border-2 border-slate-200 shrink-0">
                      <h4 class="font-bold text-slate-500 uppercase tracking-widest text-xs mb-4">Ruwe Data (Enquete)</h4>
                      <div class="flex flex-wrap gap-2">
                          <span v-for="(item, idx) in rawData" :key="idx" class="bg-slate-100 px-4 py-1 rounded-lg font-mono text-sm font-bold text-slate-600 border border-slate-300">{{ item }}</span>
                      </div>
                  </div>

                  <div class="flex-1 bg-white p-8 rounded-xl shadow-md border-4 border-slate-200 min-h-[400px] relative flex flex-col justify-end"
                      ...
                      <div class="w-full h-64 border-b-4 border-l-4 border-slate-600 relative flex items-end justify-around px-4 pb-0">
                          ...
                          <div class="w-16 bg-math-blue rounded-t-sm shadow-md border-x-2 border-t-2 border-math-blue relative z-10 transition-all duration-1000 ease-out flex justify-center items-end pb-2"

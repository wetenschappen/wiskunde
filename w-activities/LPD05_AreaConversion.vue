...
const levels = [
  {
    goalText: 'Opdracht 1: Van vierkante meter naar vierkante decimeter.',
    baseUnit: '1 m²',
    targetUnit: 'dm²',
    targetVal: 100,
    visTarget: 'dm',
  ...
  {
    goalText: 'Opdracht 2: Van vierkante decimeter naar vierkante centimeter.',
    baseUnit: '1 dm²',
    targetUnit: 'cm²',
    targetVal: 100,
    visTarget: 'dm', // We fake the grid meaning for this level
    options: [10, 100, 1000, 10000],
    hint: 'De sprong van dm² naar cm² is exact hetzelfde als van m² naar dm². De eenheid wordt 1 stapje kleiner.'
  },
  {
    goalText: 'Opdracht 3: De grote sprong! Van m² direct naar cm².',
    baseUnit: '1 m²',
    targetUnit: 'cm²',
    targetVal: 10000,
    visTarget: 'cm',
...
function checkAnswer() {
  ...
  if (droppedAns.value === currentLevelData.value.targetVal) {
    ...
    if (droppedAns.value === 10) {
        feedback.value = { type: 'error', text: 'Let op: 10 is de sprong voor LENGTE (bijv. 1 m = 10 dm). We zoeken OPPERVLAKTE (lengte × breedte).'}
    } else {
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
            ...
            <div class="p-4 mt-6 border border-slate-200 bg-slate-50 rounded-xl shadow-inner">
               ...
               <div class="flex flex-wrap gap-2">
                   <div v-for="opt in currentLevelData.options" :key="opt"
                        ...
                        :class="{'opacity-30 pointer-events-none': droppedAns === opt}">
                       {{ opt }}
                   </div>
               </div>
            </div>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-3">
              ...
              <button v-if="!isCorrect" @click="checkAnswer" :disabled="isChecked && !isCorrect || droppedAns === null" class="flex-1 py-3 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 disabled:opacity-50 active:scale-[0.98]">Controleer</button>
          ...
          <div class="flex flex-col flex-1 p-6 overflow-y-auto items-center justify-center relative pattern-grid gap-10">
              ...
              <div class="w-[300px] h-[300px] md:w-[400px] md:h-[400px] bg-orange-100 border-4 border-orange-500 shadow-xl relative transition-all duration-700">
                  ...
                  <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white/90 backdrop-blur p-4 rounded-xl shadow-lg border border-orange-200 text-center animate-fadeIn">
                      ...
                      <p v-if="visualState === 'dm'" class="font-bold text-slate-600 text-sm">10 × 10 = <span class="text-orange-500">100 blokjes</span></p>
                      <p v-if="visualState === 'cm'" class="font-bold text-slate-600 text-sm">100 × 100 = <span class="text-orange-500">10.000 blokjes</span></p>
                  </div>

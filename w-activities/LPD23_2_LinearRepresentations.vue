      ...
      <main class="flex flex-1 overflow-hidden">
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-6 border border-slate-200 bg-slate-50 rounded-xl space-y-6 shadow-inner">
              ...
              <div class="text-center bg-white p-4 border border-slate-200 rounded-xl shadow-sm">
                <p class="font-mono text-2xl font-black text-math-blue">
                  f(x) = <span class="text-math-blue">{{ a }}</span>x {{ b >= 0 ? '+ ' + b : '- ' + Math.abs(b) }}
                </p>
              </div>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-if="!isCorrect" @click="resetActivityState" class="flex-1 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">
                Probeer opnieuw
              </button>
        ...
        <div class="flex flex-col flex-1 overflow-hidden bg-slate-50">
          <div class="flex flex-col flex-1 p-6 overflow-y-auto">
            ...
            <div class="relative flex-1 flex flex-col md:flex-row items-center justify-center w-full min-h-[400px] p-8 bg-slate-100 rounded-xl border-2 border-slate-200/50 pattern-grid overflow-hidden gap-12">
              ...
              <div class="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden text-center z-10 w-48 shrink-0">
                <div class="flex bg-slate-800 text-white font-bold text-lg">
                  <div class="flex-1 p-4 border-r border-slate-600">x</div>
                  <div class="flex-1 p-4">y</div>
                </div>
                <div v-for="row in currentLevelData.table" :key="row.x" class="flex border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                  <div class="flex-1 p-4 border-r border-slate-200 font-mono font-bold">{{ row.x }}</div>
                  <div class="flex-1 p-4 font-mono font-bold text-math-blue">{{ row.y }}</div>
                </div>
              </div>

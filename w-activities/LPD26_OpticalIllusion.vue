...
const illusionTemplates = [
  {
    goalText: 'Opdracht: Het X-patroon',
    svg2D: `
  ...
  {
    goalText: 'Opdracht: Diagonale verwarring',
    svg2D: `
      <rect x="100" y="50" width="200" height="200" fill="rgba(148, 163, 184, 0.1)" stroke="#94a3b8" stroke-width="2" />
    ...
    successMsg: 'Heel goed! Deze keer was de 2D-tekening geen leugen. Zelfs in 3D-ruimte lopen deze lijnen in exact dezelfde richting (zonder te snijden).'
  }
]
...
function getHint() {
  ...
  if (attemptCount.value <= 1) {
    return 'Gebruik de "Verander Perspectief" knop om de lijnen in 3D te bekijken. Dan zie je pas de echte ligging!'
  } else if (attemptCount.value === 2) {
    if (!is3D.value) {
      return 'Je moet echt het 3D-perspectief gebruiken om het te kunnen zien. Klik op de knop!'
    }
    if (target.correctAns === 'kruisend') {
      return 'De lijnen zitten in verschillende vlakken: een vooraan en een achteraan. Ze snijden NIET en zijn NIET evenwijdig.'
    }
    return 'Let op de richting van de pijlen. Lopen ze in exact dezelfde richting?'
  }
  if (!is3D.value) return 'Klik op "Bekijk in 3D" om de ruimtelijke ligging te zien.'
  if (target.correctAns === 'kruisend') return 'Kruisend = niet evenwijdig EN niet snijdend. Ze hebben een verschillende richting maar raken elkaar niet.'
  return 'Evenwijdig = ze lopen in exact dezelfde richting, als treinsporen.'
}
...
celebrationDone.value = false
    ...
    if (levels.value.length === 0) {
      levels.value = shuffleArray(illusionTemplates)
    }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-4 mt-6 border border-surface-200 bg-math-blue-bg rounded-xl shadow-inner">
               <label class="block text-sm font-bold text-math-blue mb-2">Onderlinge ligging in 3D:</label>

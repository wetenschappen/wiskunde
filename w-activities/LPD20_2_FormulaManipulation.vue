...
// A rudimentary math state tracker specifically built for these 3 levels to avoid a full CAS
const levels = [
  {
    goalText: 'Opdracht 1: Isoleer s in de snelheidsformule.',
    targetVar: 's',
    initL: { term: 'v', fraction: null }, // v
    initR: { term: 's', fraction: 't' }   // s / t
  },
  {
    goalText: 'Opdracht 2: Isoleer m in de wet van Newton.',
    targetVar: 'm',
    initL: { term: 'F', fraction: null }, // F
    initR: { term: 'm·a', fraction: null }  // m * a
  },
  {
    goalText: 'Opdracht 3: Isoleer h in de driehoeksformule. Dit vereist 2 stappen!',
    targetVar: 'h',
    initL: { term: 'A', fraction: null }, // A
    initR: { term: 'b·h', fraction: '2' }   // b*h / 2
  }
]
...
// Input State
const opType = ref('*') // '*' or '/'
const opVar = ref('') // typed string by user
...
// Very basic symbolic manipulation for the exact cases we need
  ...
  const applySide = (state) => {
    ...
    if (opType.value === '*') {
      if (newState.fraction === v) {
        // Multiply by denominator cancels it out
      ...
      // Very crude simplifier: if dividing by something that is explicitly multiplied in the term
      const parts = newState.term.split('·');
      const idx = parts.indexOf(v);
      
      if (idx > -1) {
        // It's in the numerator, cancel it out
        parts.splice(idx, 1);
        newState.term = parts.join('·') || '1';
      } else {
        // Just add to denominator
...
function checkWin() {
  ...
  if (isolatedLeft || isolatedRight) {
    ...
    feedback.value = { 
      type: 'success', 
      text: `Uitstekend! Je hebt de formule succesvol omgevormd naar ${target}.` 
    }
  } else {
    isCorrect.value = false;
    feedback.value = { 
      type: 'info', 
      text: `De formule is correct wiskundig bewerkt. Maar ${target} staat nog niet alléén!`
    }
  }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
            ...
            <div class="p-6 border border-slate-200 bg-slate-50 rounded-xl space-y-4 shadow-inner">
              ...
              <div class="flex flex-col gap-4">
                ...
                <div class="flex gap-2">
                  <button @click="opType = '*'" class="flex-1 h-12 rounded-lg font-black text-xl border-2 transition-colors flex items-center justify-center" :class="opType === '*' ? 'border-blue-500 bg-blue-100 text-blue-700' : 'border-slate-200 bg-white text-slate-500'"><PhMultiply /></button>
                  <button @click="opType = '/'" class="flex-1 h-12 rounded-lg font-black text-xl border-2 transition-colors flex items-center justify-center" :class="opType === '/' ? 'border-blue-500 bg-blue-100 text-blue-700' : 'border-slate-200 bg-white text-slate-500'"><PhDivide /></button>
                </div>

                <div class="flex gap-2 items-center">
                   <input type="text" v-model="opVar" class="flex-1 p-3 text-lg font-bold text-slate-800 bg-white border-2 border-slate-300 rounded-lg outline-none focus:border-blue-500 text-center" placeholder="variabele of getal">
                </div>

                <button @click="applyOperation" :disabled="isCorrect || !opVar" class="w-full py-4 bg-blue-600 hover:bg-blue-500 active:scale-[0.98] text-white font-bold rounded-xl shadow-md transition-all disabled:opacity-50">
                   Toepassen op L en R
                </button>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-3">
              ...
              <button v-if="!isCorrect" disabled class="flex-1 py-3 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 opacity-50">
                Voer een actie uit
              </button>
        ...
        <div class="flex flex-col flex-1 overflow-hidden bg-slate-50">
          <div class="flex flex-col flex-1 p-6 overflow-y-auto">
            ...
            <div class="relative flex-1 flex items-center justify-center w-full min-h-[400px] p-8 bg-slate-100 rounded-2xl border-2 border-slate-200/50 pattern-grid overflow-hidden">
              ...
              <div class="bg-white p-16 rounded-3xl shadow-xl border-4 flex items-center gap-8 min-w-[400px] justify-center transition-all duration-300 transform scale-125"

...
function checkAnswer() {
  ...
  if (rootCorrect && signsCorrect) {
    ...
    feedback.value = {
      type: 'success',
      text: 'Prima!! De nulwaarde en het bijbehorende tekenschema kloppen helemaal.'
    }
  } else if (!rootCorrect) {
    isCorrect.value = false
    if (attemptCount.value >= 3) {
      ...
      feedback.value = { type: 'error', text: 'Niet helemaal... Een nulwaarde is de x-coördinaat waar de grafiek de x-as (\\(y = 0\\)) snijdt. Waar gebeurt dat?' }
    } else {
      feedback.value = { type: 'error', text: 'Niet helemaal... De nulwaarde klopt niet. Een nulwaarde is de x-coördinaat waar de grafiek de x-as (\\(y = 0\\)) snijdt.' }
    }
  } else {
    isCorrect.value = false
    if (attemptCount.value >= 3) {
      ...
      feedback.value = { type: 'error', text: `Niet helemaal... De nulwaarde klopt. Maar kijk naar de grafiek: aan de linkerkant van de nulwaarde, ligt f(x) boven of onder de x-as?` }
    } else {
      feedback.value = { type: 'error', text: 'Niet helemaal... De nulwaarde klopt, maar de plus/min tekens zijn fout. Ligt de grafiek vóór de nulwaarde onder (-) of boven (+) de x-as?' }
    }
  }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-6 border-t border-slate-200 bg-slate-50 rounded-xl space-y-6 shadow-inner">
              ...
              <div class="bg-white p-4 rounded-lg border border-slate-200 shadow-sm text-center">
                 ...
                 <span class="font-mono font-black text-xl text-math-blue ml-2">{{ currentLevelData.f_str }}</span>
              </div>
                ...
                <div class="bg-white border-2 border-slate-200 rounded-lg p-4 shadow-sm overflow-hidden">
                     <table class="w-full border-collapse text-center">
                       <tr>
                         ...
                         <td class="border-b-2 border-slate-200 font-mono font-bold text-lg text-math-blue">{{ scanX }}</td>
                         <td class="border-b-2 border-slate-200 py-2 w-16"></td>
                       </tr>
                       <tr>
                         <td class="text-sm font-bold text-slate-500 py-2 border-r-2 border-slate-200">f(x)</td>
                         <td class="py-2">
                            <button @click="toggleSign('left')" class="w-10 h-10 bg-slate-100 rounded-lg hover:bg-slate-200 font-mono text-2xl font-bold text-slate-700 border border-slate-300 transition-colors mx-auto flex items-center justify-center active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">
                              {{ userSignLeft }}
                            </button>
                         </td>
                         <td class="font-mono font-bold text-xl text-emerald-600 py-2 bg-emerald-50 rounded-lg">0</td>
                         <td class="py-2">
                            <button @click="toggleSign('right')" class="w-10 h-10 bg-slate-100 rounded-lg hover:bg-slate-200 font-mono text-2xl font-bold text-slate-700 border border-slate-300 transition-colors mx-auto flex items-center justify-center active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">
                              {{ userSignRight }}
                            </button>
                         </td>
                       </tr>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-if="!isCorrect" @click="checkAnswer" :disabled="isChecked && !isCorrect || !userSignLeft || !userSignRight" class="flex-1 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 disabled:opacity-50 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">
                Controleer
              </button>

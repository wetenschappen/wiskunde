...
function checkAnswer() {
  ...
  if (a === currentLevel.value.correctA && rest === currentLevel.value.correctRest) {
    ...
    feedback.value = {
      type: 'error',
      text: 'Niet helemaal... De waarde van a klopt niet. Als d(x) = x - a en je hebt ' + currentLevel.value.d_expr + ', wat is dan a?<br/><br/>' + getHint()
    }
  } else {
    attemptCount.value++
    isCorrect.value = false
    feedback.value = {
      type: 'error',
      text: 'Niet helemaal... Je hebt a correct, maar de rest klopt niet. Bereken P(a) door a in te vullen in de veelterm P(x).<br/><br/>' + getHint()
    }
  }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-if="!isCorrect" @click="checkAnswer" :disabled="isChecked && !isCorrect || !userA || !userRest" class="flex-1 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900 disabled:opacity-50 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-math-blue focus-visible:outline-none">
                Controleer
              </button>

    ...
    if (!isCorrect.value && draggedNumber !== null) {
        ...
        if (allPlaced) {
            ...
            if (allCorrect) {
                ...
                feedback.value = {
                    type: 'success',
                    text: 'Prima! Magistraal! Je beheerst de symbolentaal van verzamelingen perfect.'
                }
            } else {
                attemptCount.value++
                const wrongNum = required.find(n => placements.value[n] !== targets[n])
                if (attemptCount.value === 1) {
                    feedback.value = { type: 'error', text: 'Niet helemaal... Kijk goed naar de symbolen. Waar hoort het getal ' + wrongNum + ' precies thuis?' }

<script setup>
import { ref } from 'vue'
import { PhWrench, PhCalculator, PhAtom, PhX, PhFilePdf, PhEnvelope, PhCheckCircle, PhCheck, PhLink } from '@phosphor-icons/vue'
...
const periodicTableUrl = 'https://albertshalaj.github.io/lesmateriaal/pse.pdf'
...
<template>
  <div>
    ...
    <div :class="['toolbox-menu', { 'open': isOpen }]">
        ...
        <button @click="emit('open-tool', 'solutions'); toggle()" class="tool-item">
            <PhCheckCircle class="text-indigo-600 text-lg" weight="bold" /> Correctiesleutel
        </button>

        <!-- Fixed PSE Tool -->
        <button @click="openToolLink(periodicTableUrl)" class="tool-item">
            <PhAtom class="text-purple-600 text-lg" weight="bold" /> PSE
        </button>

...
import { PhPlanet, PhFlask, PhMicroscope, PhX, PhCheck } from '@phosphor-icons/vue'
...
const props = defineProps({
  ...
  subject: {
    type: String,
    default: 'physics' // 'physics' | 'chemistry' | 'science'
  }
})
...
const lpdGoalMap = [
  {
    key: 'krachtenbalans',
    label: 'Ik kan een krachtenbalans opstellen van een systeem om het effect van de resultante te bepalen (LPD F7).',
    match: /krachtenbalans|resultante|resulterende kracht|vectoren|samenstellen|ontbinden|vector/i
  },
  {
    key: 'archimedes-context',
    label: 'Ik kan de resulterende kracht verklaren in de context van de wet van Archimedes (dalen, zinken, zweven, stijgen, drijven).',
    match: /zinken|zweven|drijven|dalen|stijgen|archimedeskracht|opwaartse kracht|v_vwp|f_a/i
  },
  {
    key: 'theoretische-afleiding',
    label: 'Ik kan de wet van Archimedes theoretisch afleiden vanuit de hydrostatische druk of experimenteel aantonen.',
    match: /afleiden|hydrostatische druk|drukverschil|experimenteel|aantonen|verplaatste vloeistof|gewicht/i
  },
  {
    key: 'gas-toepassing',
    label: 'Ik kan de wet van Archimedes ook bij gassen beschouwen en toepassen, zoals bij een luchtballon.',
    match: /gassen|luchtballon/i
  }
]

const visibleGoals = computed(() => {
  const mapped = props.goals
    .map(goal => lpdGoalMap.find(item => item.match.test(goal)))
    .filter(Boolean)
  ...
  return unique.length > 0 ? unique : props.goals.map((goal, index) => ({
    key: `goal-${index}`,
    label: goal
  }))
})
...
onMounted(() => {
    ...
    if (stored) {
        ...
        completedGoals.value = parsed
            .map(item => (typeof item === 'number' ? visibleGoals.value[item]?.key : item))
            .filter(Boolean)
            ...
            <div class="bg-slate-800 text-white p-4 flex items-center justify-between shadow-lg relative z-10 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-[10px] text-slate-400 tracking-wider">{{ completedGoals.length }} / {{ visibleGoals.length }} Voltooid</div>
                    </div>
                </div>
                <!-- Close Button -->
                <button 
                        ...
                        <ul class="p-5 space-y-3">
                            ...
                            >
                                ...
                                <span 
                                    ...
                                    :class="completedGoals.includes(goal.key) ? 'text-amber-700 line-through opacity-70' : ''"
                                >
                                    {{ goal.label }}
                                </span>
                            </li>
